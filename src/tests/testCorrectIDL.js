...
// CORRECTED IDL - Fixed the game account isSigner issue
const CORRECTED_IDL = {
  "version": "0.1.0",
  ...
  "instructions": [
    {
      "name": "setWinners",
      "accounts": [
        {"name": "game", "isMut": true, "isSigner": false}, // FIXED: was true, should be false
...
async function testWithCorrectIDL() {
  console.log('🚀 Testing with CORRECTED IDL');
  console.log('==============================');
  ...
  const program = new anchor.Program(CORRECTED_IDL, PROGRAM_ID, provider);
  console.log(`Program ID: ${program.programId.toString()}`);
  
  // Test game data
  const gameId = "corrected-test-001";
  ...
  const amounts = [new anchor.BN(100000000)]; // 100 tokens
  
  console.log("Game ID:", gameId);
  console.log("Winners:", winners.map(w => w.toString()));
  console.log("Amounts:", amounts.map(a => a.toString()));
  
  // Derive game PDA - NOTE: Rust code uses `init` not `init_if_needed`
  // This means we need a NEW PDA each time, not reuse existing ones
  ...
  // Check if account exists - it MUST NOT exist for `init`
  const existingAccount = await connection.getAccountInfo(gamePda);
  if (existingAccount) {
    console.log('⚠️ Game account already exists!');
    console.log('💡 The Rust code uses `init` (not `init_if_needed`)');
    console.log('💡 You need to use a different gameId for each test');
    return;
  } else {
  ...
  try {
    ...
    // Call setWinners using corrected IDL
    ...
    console.log('\n🎉 setWinners test PASSED with corrected IDL!');
    ...
    if (error.message.includes('already in use')) {
      console.log('💡 Account already exists - use a different gameId');
    } else if (error.message.includes('signature')) {
      console.log('💡 Signature issue - check account isSigner settings');
    }
